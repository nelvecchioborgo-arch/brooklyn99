...
import type { TaskColumn } from '@/components/shared/TaskColumn';
...
export const sortTasks = (tasks: TaskColumn[], sortMode: 'chrono' | 'priority'): TaskColumn[] => {
...
export const getUpcomingTasks = (tasks: TaskColumn[], days: number = 30, limit: number = 6): TaskColumn[] => {
  const now = Date.now();
...
export const mapTasksToTasks = (allTasks: Task[], oggiStr: string): TaskColumn[] => {
  if (!allTasks || !Array.isArray(allTasks)) return [];
  const taskDaMostrare: TaskColumn[] = [];
...
export const mapDayTasksToTasks = (allTasks: Task[], targetDateStr: string): TaskColumn[] => {
  const tasksToShow: TaskColumn[] = [];
